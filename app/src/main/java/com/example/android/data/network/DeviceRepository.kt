...
class DeviceRepository {
    ...
    suspend fun getDevices(): Result<DeviceListData> {
        return try {
    ...
    suspend fun getLiveStreamStatus(deviceId: Long): Result<LiveStreamStatusData> {
        return try {
            val response = api.getLiveStreamStatus(deviceId)
            if (response.isSuccessful && response.body()?.success == true) {
                val data = response.body()!!.data
                if (data != null) Result.success(data)
                else Result.failure(Exception("데이터 없음"))
